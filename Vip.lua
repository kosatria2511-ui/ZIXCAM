-- ZIXCAM VIP - Full Features + Anti Auto Parry + Visual Wings/Halo
-- Delta Executor | Mobile + PC | Stable
...
SafeWait(function() return LocalPlayer and LocalPlayer.Character end, 5)
SafeWait(function() return LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") end, 3)
SafeWait(function() return LocalPlayer:FindFirstChild("PlayerGui") end, 3)
...
-- This forces your slash animation to be unpredictable so auto-parry scripts fail
local function AntiAutoParry()
    ...
    -- Randomize animation speed and time offset
    ...
    for _, track in pairs(tracks) do
        if track.Name and string.find(track.Name:lower(), "slash") or string.find(track.Name:lower(), "attack") or string.find(track.Name:lower(), "swing") then
            -- Random speed (0.7 - 1.5) to confuse timing-based parry
            local randomSpeed = math.random(70, 150) / 100
            track:AdjustSpeed(randomSpeed)
            
            -- Random time offset (skip ahead or rewind)
            ...
            -- Randomize animation priority to break detection
            track.Priority = Enum.AnimationPriority.Core
        end
    end
    
    -- Spoof animation ID (fake slash animation) to confuse auto-parry that reads AnimationId
    local animId = Humanoid:FindFirstChild("Animator")
    if animId then
        local fakeId = "rbxassetid://" .. tostring(math.random(100000000, 999999999))
        -- Some auto-parry scripts read the animation ID to predict parry timing
        -- By constantly changing it, they can't lock on
        for _, track in pairs(animator:GetPlayingAnimationTracks()) do
            if track.Animation then
                -- This is a spoof - actual animation data is not modified, just the ID read
...
-- Run anti-parry every frame when enabled
...
-- Also hook into attack events (if Violence District uses RemoteEvents)
-- This prevents auto-parry from reading the exact attack frame
    ...
    for _, obj in pairs(ReplicatedStorage:GetDescendants()) do
        if obj:IsA("RemoteEvent") and string.find(obj.Name:lower(), "attack") then
            ...
            obj.FireServer = function(self, ...)
                if Config.AntiAutoParry then
                    -- Add random delay before firing (0 - 50ms) to desync auto-parry
